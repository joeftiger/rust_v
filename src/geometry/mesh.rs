...
impl Mesh {
    pub fn compute_normals(&mut self) {
        ...
        for triangle in &self.triangles {
            ...
            self.vertices.index_mut(triangle.index0).normal += w0 as f32 * triangle.normal;
            self.vertices.index_mut(triangle.index1).normal += w1 as f32 * triangle.normal;
            self.vertices.index_mut(triangle.index2).normal += w2 as f32 * triangle.normal;
        }
    ...
    pub fn compute_bounding_box(&mut self) {
        let mut min = Vec3::one() * Float::INFINITY as f32;
        let mut max = Vec3::one() * Float::NEG_INFINITY as f32;
    ...
    fn angle_weights(p0: Vec3, p1: Vec3, p2: Vec3) -> (Float, Float, Float) {
        let e01 = (p1 - p0).normalized();
        let e12 = (p2 - p1).normalized();
        let e20 = (p0 - p2).normalized();

        let w0 = Float::max(-1.0, Float::min(1.0, e01.dot(-e20) as Float)).acos();
        let w1 = Float::max(-1.0, Float::min(1.0, e12.dot(-e01) as Float)).acos();
        let w2 = Float::max(-1.0, Float::min(1.0, e20.dot(-e12) as Float)).acos();
