...
impl BeckmannDistribution {
    ...
    fn beckmann_sample11(cos_theta_i: f32, sample: &Vec2) -> Vec2 {
        ...
        for it in 0..10 {
            ...
            let value = normalization * (1.0 + b + floats::FRAC_1_SQRT_PI * tan_theta_i * f32::exp(-inv_erf * inv_erf)) - sample_x;
...
impl MicrofacetDistribution for BeckmannDistribution {
    ...
    fn sample_wh(&self, wo: &Vec3, sample: &Vec2) -> Vec3 {
        ...
        if sample.x == sample.y {
            ...
            tan2_theta = -log_sample / (cos_phi * cos_phi / alpha_x2 + sin_phi * sin_phi / (self.alpha_y * self.alpha_y));
        }
